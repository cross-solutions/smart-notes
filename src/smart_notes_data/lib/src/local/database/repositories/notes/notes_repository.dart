import 'package:smart_notes_data/src/local/database/models/join_query_results.dart';
...
class NotesRepository extends BaseRepository<Notes, NoteDataObject> {
  ...
  Future<List<NoteDataObject>> selectAllOwnedNotes(String ownedBy) =>
      (select(table)..where((n) => n.ownedBy.equals(ownedBy))).get();

  Stream<List<NoteWithTag>> watchAllOwnedNotes(String ownedBy) {
    final query =
        (select(table)..where((t) => t.ownedBy.equals(ownedBy))).join([
      leftOuterJoin(
        db.tags,
  ...
  Future<void> deleteOwnedNotes(String ownedBy) =>
      (delete(table)..where((n) => n.ownedBy.equals(ownedBy))).go();

  Stream<List<NoteWithTag>> watchOwnedNotesWithTag(
      String ownedBy, String tagId) {
