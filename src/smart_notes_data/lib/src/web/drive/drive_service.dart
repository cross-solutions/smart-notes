...
import 'package:smart_notes_data/src/local/cache/keystore_service.dart';
import 'package:smart_notes_data/src/local/database/repositories/notes/notes_repository.dart';
import 'package:smart_notes_data/src/web/drive/drive_http_client.dart';
import 'package:smart_notes_data/src/web/dto/backup_dto.dart';
...
class DriveServiceImpl implements DriveService {
  ...
  Future<void> backupNotesToCloud(String ownedBy) async {
    ...
    try {
      ...
      final uploadMedia =
          Media(backupDataStream, length, contentType: '$_dataFileMimeType');
      ...
      if (driveResponse.files.isEmpty) {
        ...
        await driveApi.files.update(appDriveFile, existingBackupData.id,
  ...
  @override
  Future<void> restoreNotesFromCloud(String ownedBy) async {
    final client = await createClient();
    ...
    try {
      ...
      if (driveResponse.files.isNotEmpty) {
        Media file = await driveApi.files.get(driveResponse.files.first.id,
        ...
        await _notesRepository.deleteOwnedNotes(ownedBy);

        debugInfo('Deleting local tags...');
        await _tagsRepository.deleteOwnedTags(ownedBy);
