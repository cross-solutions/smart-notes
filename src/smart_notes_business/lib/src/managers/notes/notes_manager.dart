...
import 'package:smart_notes_business/src/mappers/notes/note_mapper.dart';
...
class NotesManagerImpl implements NotesManager {
  ...
  @override
  Stream<List<NoteEntity>> getNotesWithTagStream(
      String ownedBy, String tagId) async* {
    await for (final noteDOs
        in _notesRepository.watchOwnedNotesWithTag(ownedBy, tagId)) {
      yield noteDOs.map((n) => _noteWithTagMapper.toEntity(n)).toList();
    }
  }
}
