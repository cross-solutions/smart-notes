...
import 'package:smart_notes/models/notes/note_item_model.dart';
...
class NoteViewModel extends InitializableViewModel<NoteItemModel> {
  ...
  void onTextRecognized(void Function(String text) callback) =>
  ...
  Future<void> onStartTextRecognitionFromCamera() async {
    ...
    if (imageFile != null) {
      final recognitionResult =
          await _mlVisionService.extractTextFromImageFile(imageFile);

      if (recognitionResult.status == MLVisionResultStatus.completed &&
  ...
  Future<void> deleteNoteToEdit() async {
    final shouldDelete = await _dialogService.confirm('This cannot be undone.',
        title: 'Delete note?', ok: 'Delete');
